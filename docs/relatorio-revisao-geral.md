...
| **R-23** | **Realtime local é flaky na primeira conexão após o `db reset`** (não é a publishable key). Repro na F20-T11: logo após `supabase db reset`, `npm test` falha com os dois canais `CLOSED` e "membro NÃO recebeu o evento"; rodando de novo, passa. Na F20-T07 a 1ª execução pós-reset falhou igual e a 2ª passou; na F20-T10 (stack já aquecido) 12 cenários passaram — daí a retratação anterior ter sido precipitada. **Mitigação:** o teste de CI agora **retenta a assinatura** (até 3 tentativas com novo canal) antes de falhar, e a mensagem distingue "canal fechou" de "RLS vazou". Nenhum impacto de produto (o app reconecta sozinho — ver R-12). | `supabase/tests/realtime_test.mjs` (repro: 1ª execução pós-`db reset`) | [03](03-sincronizacao-offline.md) §4, [09](09-runbook-operacoes.md) §2.7 |
