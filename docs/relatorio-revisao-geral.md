...
| **R-12** | Canal Realtime assinado sem callback de status/erro: sem re-sync em `CHANNEL_ERROR`/`TIMED_OUT`, o cache pode defasar até o próximo bootstrap. | `lib/features/sync/data/supabase_bootstrap.dart:197-230` × `docs/03:70` | [03](03-sincronizacao-offline.md) §4 | **resolvido (F20, `4ff9d11`)** — `_aoMudarStatusCanal` re-sincroniza a cada `SUBSCRIBED` (inclusive o primeiro e após reconexão) |
...
| **R-17** | **Resolvido (migration `0016`, F24):** `convites.criado_por` sem `on delete cascade` e fora da lista de cascatas do doc 06 — hoje não aborta `excluir_conta` (só o dono cria convite), mas vira risco com a transferência de dono (Fase 6). | `0007_convites.sql:13` × `docs/06:73-74` | [01](01-banco-de-dados.md), [06](06-mvp-entregas.md) §3.3.1 |
