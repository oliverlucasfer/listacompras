...
| **R-23** | **A `sb_publishable_*` não entrega eventos no Realtime local.** Com a publishable key os canais subscrevem e fecham (`[diag] CLOSED`) sem erro visível e **nenhum** `postgres_changes` chega — inclusive ao membro; com a `ANON_KEY` (JWT legacy) o teste P-05 passa. Como o app publicado usa a publishable (`lib/core/config/supabase_config.dart`), isso pode explicar "sync em tempo real que não atualiza" no Web (F19-T02). | `supabase/tests/realtime_test.mjs` (matriz executada localmente na F20-T07) | [03](03-sincronizacao-offline.md) §4, [09](09-runbook-operacoes.md) §2.7 |
