...
| ADR-013 | 17/09/2026 *(revisado em 18/09/2026)* | **Publicação do Web suspensa por decisão do dono (18/09/2026): o Web fica para uso local.** A implementação da Fase 19 (artefatos `firebase.json`/`.firebaserc`, `/privacidade` estática, headers COOP/COEP) fica no repositório como referência, mas o **Hosting do Firebase foi desabilitado** — `https://lista-compras-34f93.web.app` responde 404. Rodar local: `flutter run -d chrome` (dev) ou `flutter build web` + servir `build/web`. A RPC `agora_servidor` e as policies continuam valendo para o banco. | ~~Publicar em URL pública (Firebase Hosting)~~ — suspenso; deploy automático no CI (F19-T03) e ajustes finais da F19 (T02/T04) ficam parados | Publicar o Web exigia Domain/Play/LGPD e manutenção de um site público sem uso previsto; o valor do offline-first está no app instalado. Nada foi excluído: reabilitar exige só um `firebase deploy` |
