...
| Tocar no item | Abre o editor (mesmo diálogo do swipe): nome, quantidade, unidade, categoria, **Preço (R$)** e ação **Remover** com undo (F12-T06/F25); a quantidade aceita frações (`1/2`, `1 1/2`, `½`, `1½`) — RF-25/F29; nome vazio/quantidade inválida/preço inválido geram erro inline no campo (F14-T07) |
...
* **Quantidades e frações (RF-25, F29):** stepper + input direto; o campo (entrada rápida e editor) aceita decimal pt-BR (`1,5`), fração (`1/2`), mista (`1 1/2`) e glifos (`½`, `1½`) via `parseQuantidade`; unidades restritas ao enum ([01 §3.1](01-banco-de-dados.md)). A exibição usa `formatarQuantidade` (`lib/features/listas/domain/quantidade.dart`): glifos comuns (`½ ¼ ¾ ⅓ ⅔`, mistos como `1½`, `1¼`) e, fora deles, arredonda para ≤ 3 casas (`1.2`, `0.143`).
