...
- **F20-T11** (R-18, R-19) — CONCLUÍDA — migration `0015_membros_insert_dono_e_convites_updated.sql` (policy de insert exige `papel in ('editor','leitor') or user_id = auth.uid()`; trigger `trg_convites_updated`) + testes em `rls_tests.sql` (N-10 ajustado à ordem nova policy→trigger, N-18, P-11, R-19 com carimbo por valor conhecido). Docs: `01 §4.4`, `02 §4.3/§5`. Descobriu que o **R-23 é real e flaky** (1ª conexão pós-`db reset`), com retry no `realtime_test.mjs`.
- **F20-T12** (R-13…R-16, R-20, R-21, R-22) — CONCLUÍDA — `main.dart` (`beforeSend` limpa breadcrumbs + contexts; `extra` ficou de fora por estar deprecado), `router.dart` (i18n + `AppSpacing`), `politica_privacidade.dart` (comentário), `pubspec.yaml` (removido `cupertino_icons`; `sqlite3` **mantido** — usado em `database_test.dart`), `app_logo.dart` (`ExcludeSemantics`, doc 15 §4), `indicador_sync.dart` (spinner 16dp), `supabase/seed.sql` (novo — 2 usuários, 1 lista, 3 itens), CSP como decisão documentada em `06 §3.4.1`. Docs: `06`, `07 §4`, `09 §2.4`; teste do logo atualizado.
