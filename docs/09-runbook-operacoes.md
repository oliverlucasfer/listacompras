...
- **Fase 38 — Notificações push (23/09/2026):** migrations **`0021_push_tokens.sql`** e **`0022_notificar_push.sql`** aplicadas em produção (`supabase db push`, aditivas); Edge Function **`enviar-push`** publicada (`supabase functions deploy enviar-push --no-verify-jwt`); secret **`PUSH_WEBHOOK_SECRET`** setado e segredos do Vault **`push_function_url`**/**`push_webhook_secret`** criados (URL `https://smshgctdwxkqbvbdlhud.supabase.co/functions/v1/enviar-push`). Smoke do gateway: `POST` sem o segredo → `401 unauthorized` (fail-closed). **Pendente:** `FCM_SERVICE_ACCOUNT` (service account do FCM no console do Firebase) — sem ela a função é no-op e nenhuma notificação é enviada; smoke real com 2 aparelhos e nova distribuição aos testadores (F5-T05b) ficam para depois.
