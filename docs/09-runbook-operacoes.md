...
- **Fase 7 (compartilhamento por link, 10/09/2026):** app `1.1.1+4` (convites por link + RPC `aceitar_convite` + Realtime de `lista_membros`, [08](08-compartilhamento-colaborativo.md) §2–§7) distribuído ao grupo `testadores` (App Distribution); sem mudanças na Edge Function `parse-lista`. **Migrations `0007_convites.sql` e `0008_lista_membros_replica_identity.sql` AINDA NÃO aplicadas em produção** (`supabase migration list` divergente — local 0001–0008, remote 0001–0006): sem elas o compartilhamento não funciona em produção; aplicar via `supabase db push` (runbook §2.4) — pendência do dono registrada no fechamento da F7-T07. Validação física em dispositivos (deep link de convite e perda de acesso < 5s, 08 §9 itens 1 e 5) fica pendente; verificações automatizadas cobrem os itens do recorte (suites SQL N/E/A + 217 testes Flutter).
