...
- **Fase 7 (compartilhamento por link, 10/09/2026):** migrations `0007_convites.sql`, `0008_lista_membros_replica_identity.sql` e `0009_rls_membros_papeis.sql` aplicadas em produção via `db push` (convites/RLS/RPC `aceitar_convite` + identity full para o Realtime devolver `user_id` em DELETE + policies de mudança de papel e saída voluntária, [08](08-compartilhamento-colaborativo.md) §2–§7); sem mudanças na Edge Function `parse-lista`; APK `1.1.1+4` distribuído e **substituído por `1.1.2+5`** após o final review corrigir RLS faltante (mudar papel/sair eram no-ops), whitelist de tabelas no realtime e limpeza local no sair — minors deferridos registrados no ledger da fase. Validação física em dispositivos (deep link de convite e perda de acesso < 5s, 08 §9 itens 1 e 5) fica pendente com os testadores; verificações automatizadas cobrem o recorte (suites SQL rls/excluir_conta/aceitar_convite + 223 testes Flutter).
