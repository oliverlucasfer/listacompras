...
* **Payload de listas inclui `orcamento_centavos`** (orçamento da lista em centavos [01 §4.1](01-banco-de-dados.md), RF-28/F36). Diferente de `arquivada_em`, é enviado **sempre** (como `titulo`): a mescla do coalescing é por chave e o campo não conflita com o arquivo. `null` = sem orçamento; `0` é válido. O aplicador trata a coluna com **tolerância**: ausente ou não numérica → `null` (linha gravada por app antigo). A coluna herda o RLS de `listas` (sem policy nova).
* **O histórico de preços (`HistoricoPrecoLocal`) é local-only** (RF-29/F37): tabela **Drift por dispositivo** que guarda o último preço pago por item. **Não** entra na fila (`mutacao_pendente`), no payload nem no aplicador/LWW — diferentemente das colunas de `listas`/`itens_lista` acima, o registro ao marcar o item como comprado **não gera mutação remota** e não é compartilhado entre membros/dispositivos.
* **Importação de backup (RF-31/F41) alimenta a fila no modo colaborativo:** cada lista/item **efetivamente gravado** (os descartados pelo LWW por `updated_at` mais antigo são pulados e **não** enfileirados) entra como `INSERT` com o `updated_at` importado no payload — é esse valor que decide o LWW no servidor, preservando o timestamp original; `ts_local` recebe o instante do enfileiramento. O enfileiramento ocorre **na mesma transação** da gravação: importação que falha não deixa mutação órfã. No modo Lite (`enfileirar == false`, sem sync) a importação é local-only e **não** enfileira, como toda escrita do Lite; o histórico de preços nunca é enfileirado em nenhum modo (local-only, acima).
